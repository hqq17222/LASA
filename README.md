...
> 📖 **新同学请先读 [学生上手指南](学生上手指南.md)**：环境搭建、服务器部署、App 打包、常见坑全在里面。

## 系统组成

| 目录 | 说明 |
|---|---|
| `backend/` | FastAPI 后端：用户权限、生态评估指标、地图图层、外业照片（EXIF 自动解析定位 + GLM-4V 植物识别）、语音备注、轨迹、样地、样地调查记录、外业看板等 API |
| `frontend/` | Vue 3 + Vite + Leaflet 前端：监测大屏、生态评估、地图、巡检照片、用户管理、使用说明等；`/m` 为 App 专属移动端外业页（找样地/拍照识别/样地调查/轨迹） |
| `android-app/` | 安卓外业 App（Kotlin）：WebView 壳加载 `/m` 页 + 前台轨迹记录服务 + JS 桥。**定位：只管外业采集（植物识别、考察打点、样地调查、轨迹跟踪、样地矢量图层导航）；分析展示、气象/物联网监测由网页大系统承担** |
