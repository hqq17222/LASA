"""停后端腾内存 + 限制 node 堆大小构建前端，完成后恢复后端。"""
import os
import paramiko
...
CMDS = [
    ...
    "cd /opt/lasa-nanshan-platform/frontend && NODE_OPTIONS='--max-old-space-size=512' npx vite build > /tmp/build_cap.log 2>&1; echo EXIT=$?; tail -8 /tmp/build_cap.log",
