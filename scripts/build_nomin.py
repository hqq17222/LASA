"""服务器免压缩构建（避开 esbuild minify 内存峰值），恢复网站。"""
import os
import paramiko
...
for cmd in [
    ...
    "cd /opt/lasa-nanshan-platform/frontend && NODE_OPTIONS='--max-old-space-size=512' npx vite build --minify false > /tmp/build_nomin.log 2>&1; echo EXIT=$?; tail -5 /tmp/build_nomin.log",
