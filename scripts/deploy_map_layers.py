"""上传 PatrolPhotos.vue（底图切换/矢量图层/我的位置）-> 停后端重建前端 -> 恢复。"""
import os
import paramiko
...
sftp = c.open_sftp()
sftp.put(os.path.join(ROOT, "frontend", "src", "views", "PatrolPhotos.vue"),
         f"{REMOTE}/frontend/src/views/PatrolPhotos.vue")
print("已上传 PatrolPhotos.vue")
sftp.close()

CMDS = [
    "supervisorctl stop lasa-nanshan-backend; sleep 2; "
    "cd /opt/lasa-nanshan-platform/frontend && NODE_OPTIONS='--max-old-space-size=512' npx vite build > /tmp/build_map.log 2>&1; echo EXIT=$?; tail -5 /tmp/build_map.log",
    ...
    "curl -s -o /dev/null -w 'static:%{http_code}' http://127.0.0.1:18480/static/patrol_1_8e409c36a82d.jpg",
]
for cmd in CMDS:
